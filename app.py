...
def get_url(query_string):
    uri = 'https://api.twitter.com/1.1/search/tweets.json?q={}'.format(query_string)
    return uri
...
def twitter_auth():
    ...
    if not user:
        user = User(screen_name=access_token['screen_name'], oauth_token=access_token['oauth_token'],
                    oauth_token_secret=access_token['oauth_token_secret'], id=None)
        user.save_to_db()

    session['screen_name'] = user.screen_name

    return redirect(url_for('profile')) # Name of the method and not the end point
